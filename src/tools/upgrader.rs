...
use crate::tools::utils::{get_latest_tag, run_shell_command};
...
pub fn upgrade() {
    println!("Do you want to upgrade cmdcreate?\n(Y or N)");
    ...
    match method.trim() {
        "1" => run_shell_command("yay -Syyu cmdcreate", || return),
        "2" => {
        ...
        "3" => {
            ...
            let file_to_download = format!("cmdcreate-{}-linux-bin", latest_release);
                ...
                .find(|a| a.name == file_to_download)
            {
                ...
                println!("Binary not found in latest release.");
            }
        }
