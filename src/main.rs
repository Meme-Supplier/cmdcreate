...
pub static PROJ_VER: &str = "v0.6.0";
    ...
    match args[0].as_str() {
        ...
        "--license" | "-l" => {
            if args_contains("--offline") {
                ...
                run_shell_command("curl -s https://raw.githubusercontent.com/Meme-Supplier/cmdcreate/master/LICENSE",
                || run_shell_command("cat ~/.local/share/cmdcreate/LICENSE", || println!("Error: Unable to display license.")))
            }
        }
        ...
        "--changelog" | "-c" => {
            if args_contains("--offline") {
                ...
                run_shell_command("curl -s https://raw.githubusercontent.com/Meme-Supplier/cmdcreate/master/changes.md",
                || run_shell_command("cat ~/.local/share/cmdcreate/changes.md", || println!("Error: Unable to display changelog.")))
            }
        }
        ...
        _ => {
            if args[0].starts_with("-") {
                error("Invalid argument:", &format!("{}", args[0]));
            } else {
