...
pub fn upgrade() {
    ...
    match method.trim() {
        ...
        "2" => {
            ...
            match get_latest_tag("Meme-Supplier", "cmdcreate") {
                Ok(latest_release) => {
                    run_shell_command(&format!("curl -L -o /tmp/cmdcreate-{latest_release}-linux.deb https://github.com/Meme-Supplier/cmdcreate/releases/latest/download/cmdcreate-{latest_release}-linux.deb"), || {});
                    run_shell_command(&format!("sudo dpkg -i /tmp/cmdcreate-{latest_release}-linux.deb"), || {});
                }
